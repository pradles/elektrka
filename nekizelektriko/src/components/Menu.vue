...
<button @click="toggleSidebar" aria-controls="logo-sidebar" type="button" class="inline-flex items-center mt-2 ms-3 text-sm text-gray-500 rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200">
...
<aside id="default-sidebar" :class="{'-translate-x-full': !isSidebarOpen, 'translate-x-0': isSidebarOpen}" class="fixed top-0 sm:left-12 left-0 z-40 sm:w-72 w-64 h-screen transition-transform sm:transition-transform sm:duration-500 bg-white" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 sm:border-l-4 border-l-0 border-r-4 border-gray-200">
        ...
        <ul class="space-y-2 font-medium mt-20">
            <hr>
            <li>
            <a @click="toggleSidebar" href="home" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group">
            ...
            <li>
            <a @click="toggleSidebar" href="users" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group">
                <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
